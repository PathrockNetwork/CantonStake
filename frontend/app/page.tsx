        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-px bg-ink-700">
          {[
            ...
            },
            {
              phase: "bonded",
              title: "Bond",
              canton: "StakingPosition · Bonded",
              polygon: "buyVoucher → ShareMinted",
              marker: true,
            },
            {
              phase: "unbonding",
              title: "Unbond",
              canton: "ConfirmUnbond · Unbonding",
              polygon: "sellVoucher_new → ShareBurned",
              marker: true,
            },
            {
              phase: "released",
              title: "Release",
              canton: "Release · Released",
              polygon: "unstakeClaimTokens_new",
              marker: false,
            },
          ].map((step, i) => (
            <div
      ...
      <section className="grid grid-cols-12 gap-8">
        ...
        <div className="col-span-12 md:col-span-8 space-y-6 text-ink-200 leading-relaxed">
          <p>
            ...
            attest on-ledger that an economically meaningful event happened —
            a bond, an unbond, a settlement. Super Validator automation
            converts each marker into an <em>AppRewardCoupon</em>, which mints
            Canton Coin in the next 10-minute round.
          </p>
          <p>
            CantonStake uses markers only where the CIP-47 guidance permits:
            on state transitions that represent real value movement. We do
            not mark intermediate propose steps.
          </p>
          <p>
            The beneficiary split is defined in the Daml contract itself:
            75% of each marker's reward weight flows directly to the
            delegator's Canton party, 25% to the app treasury. No custodial
            middleman, no off-ledger accounting.
          </p>
        </div>
      </section>
    </div>
